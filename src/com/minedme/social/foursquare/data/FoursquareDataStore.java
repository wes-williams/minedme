package com.minedme.social.foursquare.data;
...
public class FoursquareDataStore extends DataStore
{
	private final static Logger logger = Logger.getLogger(FoursquareDataStore.class.getName());
	
	private static final String NETWORK_NAME = "FOURSQUARE";
	private static final String USER_URL ="https://api.foursquare.com/v2/users/self";
	private static final String CHECKIN_URL = "https://api.foursquare.com/v2/users/self/checkins";
/*
	private static final String API_KEY = "XXXXXXXXXXXXXXXXX";
	private static final String API_SECRET = "XXXXXXXXXXXXXXXXX";
	private static final String API_CALLBACK_URL = "https://mined-me.appspot.com/fsCallback";
*/
	private Network network;
	private OAuthService service;
	
	public FoursquareDataStore()
	{
	}
...
/*		
		if(network == null)
		{
			...
			{
				logger.warning("Failure creating FOURSQUARE network");
			}
		}
*/
	...
	public static String createCheckinUrl(String oauthToken)
	{
		Calendar cal = Calendar.getInstance();

		String beforeTimestamp = "beforeTimestamp=" + cal.getTimeInMillis();
		beforeTimestamp = beforeTimestamp.substring(0,beforeTimestamp.length()-3);
		cal.add(Calendar.DATE, -1);
		String afterTimestamp = "afterTimestamp=" + cal.getTimeInMillis();
		afterTimestamp = afterTimestamp.substring(0,afterTimestamp.length()-3);
				
		return CHECKIN_URL + "?limit=1&" + beforeTimestamp + "&" + afterTimestamp + "&oauth_token=" + oauthToken;
	}
	
	public static String createUserUrl()
	{
		return USER_URL + "?oauth_token=";
	}
	
	public FoursquareUserData retrieveUserData(UserNetwork userNetwork) 
	{		
		FoursquareUserData userData = null;
		
		try
		{
			String decryptedJson = EncryptionUtil.decrypt(userNetwork.getPrivateData());
			ObjectMapper mapper = new ObjectMapper();
			userData = mapper.readValue(decryptedJson, FoursquareUserData.class);
		}
		catch(Throwable t)
		{
	...
	public boolean storeUserData(UserData userData,UserNetwork userNetwork)  
	{
		try
		{
			FoursquareUserData fsUserData = (FoursquareUserData) userData;
